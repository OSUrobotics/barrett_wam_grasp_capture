...
#include "nonstandard_main_function.h"

#include <boost/thread.hpp> // BarrettHand threading
#include <boost/bind.hpp>
#include <boost/foreach.hpp>

#include <bullet/LinearMath/btMatrix3x3.h> // Bullet includes
...
#include "wam_msgs/HandCommand.h"
...
#include "wam_srvs/JointRecordPlayback.h"
#include "wam_srvs/HandCommandBlk.h"
...
const int PUBLISH_FREQ = 250; // Default Control Loop / Publishing Frequency
const int BHAND_PUBLISH_FREQ = 10; // Publishing Frequency for the BarretHand
...
template<size_t DOF>
  ...
  protected:
    ...
    ros::Subscriber hand_command_sub;
    ros::Subscriber hand_open_grsp_sub, hand_close_grsp_sub;
    ...
    //Recording and Playback State Vars
    WamRecorder<DOF>* wam_rec;
  ...
  public:
    ...
    void
    handOpenGrasp(const std_msgs::Empty::ConstPtr &msg);
    void
    handCloseGrasp(const std_msgs::Empty::ConstPtr &msg);
    bool
    ...
    bool
    commandHandBlocking(wam_srvs::HandCommandBlk::Request &req, wam_srvs::HandCommandBlk::Response &res);
    void 
    moveHand(const wam_msgs::HandCommand msg, bool blocking);
    ...
    void 
    commandHand(const wam_msgs::HandCommand::ConstPtr& msg);
    void
...
public:
	WamRecorder(){
		ROS_ERROR("WamRecorder must have a wam and a wamp at startup.");
		exit(1);
	};

	WamRecorder(WamNode<DOF>* wam_node, systems::Wam<DOF>* wam, systems::Ramp* ramp, jp_type jp_home, Hand* hand) {
		this->ramp = ramp;
		this->wam = wam;
		this->jp_home = jp_home;
		this->hand = hand;
		this->wam_node = wam_node;
    		ros::NodeHandle gc_nh_("wam_grasp_capture");
    		gc_nh_.setCallbackQueue(&record_playback_queue);
    		
		spinny = new ros::AsyncSpinner(2, &record_playback_queue);
    		spinny->start();
    		
		recording = false;
    		playing_back = false;
		hand_recording = false;
    		hand_playing_back = false;
		loaded = false;
    		start_jnt_bag_record_sub = gc_nh_.subscribe("jnt_record_start", 1, &WamRecorder::jointRecordStartCB, this);
    		stop_jnt_bag_record_sub = gc_nh_.subscribe("jnt_record_stop", 1, &WamRecorder::jointRecordStopCB, this);
    		load_jnt_playback_srv = gc_nh_.advertiseService("jnt_playback_load", &WamRecorder::jointPlaybackLoadCB, this);
    		start_jnt_bag_playback_sub = gc_nh_.subscribe("jnt_playback_start", 1, &WamRecorder::jointPlaybackStartCB, this);
    		stop_jnt_bag_playback_sub = gc_nh_.subscribe("jnt_playback_stop", 1, &WamRecorder::jointPlaybackStopCB, this);
    		
		/*start_hand_bag_record_sub = gc_nh_.subscribe("hand_record_start", 1, &WamRecorder::handRecordStartCB, this);
    		stop_hand_bag_record_sub = gc_nh_.subscribe("hand_record_stop", 1, &WamRecorder::handRecordStopCB, this);
    		start_hand_bag_playback_sub = gc_nh_.subscribe("hand_playback_start", 1, &WamRecorder::handPlaybackStartCB, this);
    		stop_hand_bag_playback_sub = gc_nh_.subscribe("hand_playback_stop", 1, &WamRecorder::handPlaybackStopCB, this);*/
	...
	void jointRecordStartCB(const std_msgs::String::ConstPtr& msg);
	void jointRecordStopCB(const std_msgs::Empty::ConstPtr& msg);
	bool jointPlaybackLoadCB(wam_srvs::JointRecordPlayback::Request& req, wam_srvs::JointRecordPlayback::Response& res);
	void jointPlaybackStartCB(const std_msgs::Empty::ConstPtr& msg);
	void jointPlaybackStopCB(const std_msgs::Empty::ConstPtr& msg);
	/*void handRecordStartCB(const std_msgs::String::ConstPtr& msg);
	void handRecordStopCB(const std_msgs::Empty::ConstPtr& msg);
	void handPlaybackStartCB(const std_msgs::String::ConstPtr& msg);
	void handPlaybackStopCB(const std_msgs::Empty::ConstPtr& msg);*/

private:
	systems::Wam<DOF>* wam;
	WamNode<DOF>* wam_node;
	systems::Ramp* ramp;
	Hand* hand;
	jp_type jp_home;
	
	ros::Subscriber start_jnt_bag_record_sub, stop_jnt_bag_record_sub;
	ros::Subscriber start_jnt_bag_playback_sub, stop_jnt_bag_playback_sub;
	ros::ServiceServer load_jnt_playback_srv;
	//ros::Subscriber start_hand_bag_record_sub, stop_hand_bag_record_sub;
	//ros::Subscriber start_hand_bag_playback_sub, stop_hand_bag_playback_sub;
    	
	sensor_msgs::JointState bhand_joint_state;

    	ros::AsyncSpinner* spinny;
    	ros::CallbackQueue record_playback_queue;
    	bool loaded;
	bool recording, hand_recording;
    	bool playing_back, hand_playing_back;
	math::Spline< jp_type > *spline;
	systems::Callback<double, jp_type> *trajectory;
};
...
math::Vector<3>::type toRPY(Eigen::Quaterniond inquat)
{
  ...
  btMatrix3x3(q).getEulerZYX(y, p, r);
  newRPY[2] = y;
  newRPY[1] = p;
  newRPY[0] = r;
  return newRPY;
...
template<size_t DOF>
  void WamNode<DOF>::init(ProductManager& pm)
  {
    ...
    if (pm.foundHand()) //Does the following only if a BarrettHand is present
    {
      ...
      hand_open_grsp_sub = nh_.subscribe("open_grasp", 1, &WamNode<DOF>::handOpenGrasp, this); // bhand/open_grasp
      hand_close_grsp_sub = nh_.subscribe("close_grasp", 1, &WamNode::handCloseGrasp, this); // bhand/close_grasp
      ...
      hand_sprd_vel_srv = nh_.advertiseService("hand_cmd_blocking", &WamNode::commandHandBlocking, this); // bhand/spread_vel
    
      hand_command_sub = nh_.subscribe("hand_cmd", 1, &WamNode::commandHand, this); // wam/cart_pos_cmd
    ...
    ROS_INFO("Enabled gravity compensation for the wam_node.");
    ...
    wam_rec = new WamRecorder<DOF>(this, &wam, &ramp, jp_home, hand);
  }
...
template<size_t DOF>
  bool WamNode<DOF>::gravity(wam_srvs::GravityComp::Request &req, wam_srvs::GravityComp::Response &res)
  {
    ...
    ROS_INFO("Gravity compensation enabled? %s", (wam.isGravityCompensated()) ? "true" : "false");
...
template<size_t DOF>
  bool WamNode<DOF>::goHome(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
  {
    ...
    ROS_INFO("Finished moving to straight up position.");
    //jp_home[3] -= 0.3;
    //wam.moveTo(jp_home, true);
    //jp_home[3] += 0.3;
    wam.moveTo(jp_home, true);
    ROS_INFO("Completed motion to home position.");
    return true;
  }
...
template<size_t DOF>
  ...
  {
    // The WAM cant be taking other commands while it is playing back a trajectory
    //if (playing_back) {
//	return;
  //  }
...
template<size_t DOF>
  void WamNode<DOF>::handOpenGrasp(const std_msgs::Empty::ConstPtr &msg)
  {
...
template<size_t DOF>
  void WamNode<DOF>::handCloseGrasp(const std_msgs::Empty::ConstPtr &msg)
  {
...
template<size_t DOF>
  bool WamNode<DOF>::commandHandBlocking(wam_srvs::HandCommandBlk::Request &req, wam_srvs::HandCommandBlk::Response &res)
  {
    moveHand(req.command, true);
    return true;
  }
template<size_t DOF>
  void WamNode<DOF>::commandHand(const wam_msgs::HandCommand::ConstPtr& msg)
  {
    /*double f1[3] = {0,0,0};
    double f2[3] = {0,0,0};
    double f3[2] = {0,0};
    for (unsigned int i = 0; i < msg->name.size(); ++i) {
	if (msg->name[i] == "finger_1_prox_joint") {
		f1[0] = msg->position[i];
	} else if (msg->name[i] == "finger_1_med_joint") {
		f1[1] = msg->position[i];
	} else if (msg->name[i] == "finger_1_dist_joint") {
		f1[2] = msg->position[i];
	} else if (msg->name[i] == "finger_2_prox_joint" ) {
		f2[0] = msg->position[i];
	} else if (msg->name[i] == "finger_2_med_joint") {
		f2[1] = msg->position[i];
	} else if (msg->name[i] == "finger_2_dist_joint") {
		f2[2] = msg->position[i];
	} else if (msg->name[i] == "finger_3_med_joint") {
		f3[0] = msg->position[i];
	} else if (msg->name[i] == "finger_3_dist_joint") {
		f3[1] = msg->position[i];
	} else {
		ROS_ERROR_STREAM("Unknown joint name: " << msg->name[i]);
	}
    }*/
    moveHand(*msg, false);
    return;
...
template<size_t DOF>
  void WamNode<DOF>::moveHand(const wam_msgs::HandCommand msg, bool blocking)
  {
    hand->trapezoidalMove(Hand::jp_type(msg.f1, msg.f2, msg.f3, 0.0), Hand::GRASP, blocking);
    hand->trapezoidalMove(Hand::jp_type(msg.spread), Hand::SPREAD, blocking);
  }


////////// The Start of WAM modifications
template<size_t DOF>
  void WamRecorder<DOF>::jointRecordStartCB(const std_msgs::String::ConstPtr& msg)
  {
    if (recording || playing_back) {
      ROS_ERROR("Already playing back a trajectory or recording one.");
      return;
    }
    
    ROS_INFO_STREAM("Beginning joint record to bag file: " << msg->data);
    wam->idle();
    wam->gravityCompensate(true);

    rosbag::Bag bag;
    bag.open(msg->data.c_str(), rosbag::bagmode::Write);
    ...
    unsigned int i;
    recording = true;
    ros::Rate freq(PUBLISH_FREQ); // Record at 1/10 the frequency
    while (recording && ros::ok()) {
      cur_jnt = wam->getJointPositions();
      for (i = 0; i < DOF; ++i) {
	js.position[i] = cur_jnt[i];
      }

      js.header.stamp = ros::Time::now();
      bag.write("wam_jnts", ros::Time::now(), js);
      freq.sleep();
    }

    ROS_INFO_STREAM("Stopping joint record for " << msg->data);
    bag.close();
    recording = false;
    return;
  }
...
template<size_t DOF>
  void WamRecorder<DOF>::jointRecordStopCB(const std_msgs::Empty::ConstPtr& msg)
  {
    ROS_INFO("Joint record stop received.");
    if (!recording) {
      ROS_ERROR("Not recording a trajectory!");
      return;
    }

    recording = false;
    /*res.ret = true;
    res.err_code = res.SUCCESS;*/
    return;
  }

template<size_t DOF>
bool WamRecorder<DOF>::jointPlaybackLoadCB(wam_srvs::JointRecordPlayback::Request& req, wam_srvs::JointRecordPlayback::Response& res)

  {
    if (playing_back || recording) {
      ROS_ERROR("Already reciting or recording a trajectory");
      res.ret = false;
      res.err_code = res.SERVICE_ALREADY_RUNNING;
      return true;
    }

    ROS_INFO_STREAM("Beginning joint playback from bag file: " << req.data_file);
    playing_back = true;

    rosbag::Bag bag;
    bag.open(req.data_file.c_str(), rosbag::bagmode::Read);
    std::vector<std::string> topics;
    topics.push_back(std::string("wam_jnts"));
    rosbag::View view(bag, rosbag::TopicQuery(topics));

    sensor_msgs::JointState::ConstPtr js;
    boost::tuple<double, jp_type> cur_jnt;
    std::vector <boost::tuple<double, jp_type> > joint_pos;
    unsigned int i = 100;
    ros::Time base_time;
    ros::Time last_time(0);
    BOOST_FOREACH(rosbag::MessageInstance const m, view) {
      js = m.instantiate<sensor_msgs::JointState>();
      if (i == 100) {
	base_time = js->header.stamp;
      }
      
      // Enforce time monoticity for spline
      if (js->header.stamp.toSec() <= last_time.toSec()) {
	continue;
      }

      // Set jp_type joint state properties
      boost::tuples::get<0>(cur_jnt) = ((js->header.stamp - base_time).toSec() + 0.006) * SPEED;
      for (i = 0; i < DOF; ++i) {
      	boost::tuples::get<1>(cur_jnt)[i] = js->position[i];
      }

      // Add joint to list of joints
      last_time = js->header.stamp;
      joint_pos.push_back(cur_jnt);
    }

    ROS_INFO_STREAM("Completed bag file read. Preparing spline. First time: " << boost::tuples::get<0>(joint_pos[0]) << " last time: " << boost::tuples::get<0>(joint_pos[joint_pos.size()-1]));
    ROS_INFO_STREAM("Spline info: points: " << joint_pos.size() << " base time: " << base_time.toSec());

    // Make the spline
    spline = new math::Spline< jp_type >(joint_pos);
    ramp->stop();
    ramp->reset();
    ...
    ROS_INFO("Setting up Trajectory execution.");
    trajectory = new systems::Callback<double, jp_type> (boost::ref(*spline));
    connect(ramp->output, trajectory->input);
...
template<size_t DOF>
  void WamRecorder<DOF>::jointPlaybackStartCB(const std_msgs::Empty::ConstPtr& msg)
{
    if (!loaded) {
	ROS_ERROR("No trajectory loaded. Skipping.");
	return;
    }

    ROS_INFO("Starting trajectory execution.");
    ramp->start();
    while (trajectory->input.getValue() < spline->finalS() && playing_back) {
      usleep(100000);
    }
    ...
    ROS_INFO_STREAM("Exiting jointPlayback callback.");
    return;
  }

template<size_t DOF>
  void WamRecorder<DOF>::jointPlaybackStopCB(const std_msgs::Empty::ConstPtr& msg)
  {
    ROS_INFO("Joint playback stop called.");
    if (!playing_back) {
      ROS_ERROR("Not playing back a trajectory.");
      return;
    }

    // Idle the WAM
    ramp->stop(); // Stop the playback
    ramp->reset();
    wam->idle(); // Disconnect the trajectory input
    wam->moveTo(jp_home, true);
